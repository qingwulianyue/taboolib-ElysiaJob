...
class PlayerSkillDataManager {
    ...
    fun getPlayerSkillRemainCoolDown(uuid: UUID, skillId: String): Double {
        // 如果玩家没有技能数据，则返回0
        if (!skillCooldownDataMap.containsKey(uuid)) return 0.0
        if (!skillCooldownDataMap[uuid]!!.containsKey(skillId)) return 0.0
        // 获取技能数据，若技能不存在返回0
        val skillData = ElysiaJob.skillDataManager.getSkillData(skillId) ?: return 0.0
